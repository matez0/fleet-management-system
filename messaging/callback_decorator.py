"""Source: https://github.com/matez0/rabbitmq-callback-decorator"""
...
class ErrorHandlerCallback(ABC):
    ...
    def __init__(self, do_callback: Callable[[bytes, dict], None]):
...
class JsonCallback:
    def __init__(self, do_callback: Callable[[dict, dict], None]):
        self.do_callback = do_callback

    def __call__(self, body: bytes, headers: dict):
        try:
...
class ModelCallback:
    ...
    def __call__(self, do_callback: Callable[[BaseModel, dict], None]):
        def wrapper(body: dict, headers: dict):
...
class CallbackMeta(type(ABC)):
    ...
    def __call__(cls, do_callback: Callable[[bytes, dict], None]):
    ...
    def _(cls, message_type: type(BaseModel), *message_types):
        ...
        def wrap(do_callback: Callable[[BaseModel, dict], None]):
